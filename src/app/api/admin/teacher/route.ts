...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!session || session.role !== 'ADMIN') return NextResponse.json({ error: 'Akses ditolak' }, { status: 403 });
  ...
  } catch { return NextResponse.json({ error: 'Terjadi kesalahan server' }, { status: 500 }); }
}
